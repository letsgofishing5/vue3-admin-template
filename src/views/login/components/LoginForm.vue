<template>
    <BasicForm :json-conf="formConf" v-model="loginUser">
        <template #password="{ props, events }">
            <el-input @keyup.enter="events.submit(submitForm)" v-bind="props" v-model="loginUser.password" />
        </template>
        <template #btns="{ events }">
            <el-button class="w-full" type="primary" @click="events.submit(submitForm)">登录</el-button>
        </template>
    </BasicForm>
</template>
